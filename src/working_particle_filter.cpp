...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
    ...
    /*for (int i=0; i<num_particles; i++) {
        ...
        default_random_engine gen;
        normal_distribution<double> N_x_init(p.x, std_pos[0]);
        normal_distribution<double> N_y_init(p.y, std_pos[1]);
        normal_distribution<double> N_theta_init(p.theta, std_pos[2]);
        p.x = p.x + (velocity/yaw_rate)*(sin(p.theta + yaw_rate*delta_t) - sin(p.theta)) + N_x_init(gen);
        p.y = p.y + (velocity/yaw_rate)*(cos(p.theta) - cos(p.theta + yaw_rate*delta_t)) + N_y_init(gen); 
        p.theta = p.theta + yaw_rate*delta_t + N_theta_init(gen);
    }*/
